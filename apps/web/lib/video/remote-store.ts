...
export async function getRemoteObject(objectPath: string): Promise<Response | null> {
  ...
  if (response.status === 404) return null;  if (response.status === 400) {    const detail = await response.text();    if (detail.includes("Object not found")) return null;    throw new Error("Render storage read failed (" + response.status + "): " + detail.slice(0, 200));  }
