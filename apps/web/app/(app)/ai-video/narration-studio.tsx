...
export function NarrationStudio() {
  ...
  async function generate() {
    ...
    try {
      const response = await fetch("/api/v1/video-jobs", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ script, format: "portrait", quality, useAvatar, sceneMediaUrls, renderer: "remotion", allowRendererFallback: false }) });
...
export function NarrationStudio() {
  ...
  async function generate() {
    ...
    setError(undefined); setJob(undefined);
    try {
      const response = await fetch("/api/v1/video-jobs", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ script, format: "portrait", quality, useAvatar, sceneMediaUrls }) });
      const body = await readJson<VideoJob & { detail?: string }>(response);
      if (!response.ok) throw new Error(body.detail ?? "Video job could not be created.");
      setJob(body); void poll(body.id);
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Video job could not be created."); }
