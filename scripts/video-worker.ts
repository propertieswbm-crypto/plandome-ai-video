  ...
  try {
    ...
    if (isHmoProject) {
      for (const [index, scene] of scenes.entries()) {
        ...
        const canonicalAsset = creativeProject.assets.find((asset) => asset.sceneId === canonicalScene?.id);
        const semanticScore = Number(canonicalAsset?.semanticScore ?? (attributions[index]?.fallback ? .8 : .86));
        const mustRepairSemantics = semanticScore < .84;
        const curatedSource = path.join(
          root,
          "apps",
          "web",
          "public",
        ...
        const currentAsset = scene.videoAsset || scene.visualAsset;
        const currentSource = currentAsset ? path.join(assets, path.basename(currentAsset)) : undefined;
        const selectedSource = mustRepairSemantics || !currentSource ? curatedSource : currentSource;
        try {
          await stat(selectedSource);
        } catch {
          continue;
        }
        ...
        await exec(mediaBinary("ffmpeg"), [
          "-y",
          ...(imageInput ? ["-loop", "1"] : ["-stream_loop", "-1"]),
          "-i", selectedSource,
          "-t", Math.max(scene.duration, .5).toFixed(6),
          "-an",
          ...
          "-movflags", "+faststart",
          repairedPath
        ], { maxBuffer: 10_000_000, timeout: 180_000 });

        scene.videoAsset = repairedName;
        scene.visualAsset = undefined;
        if (canonicalAsset) {
          canonicalAsset.uri = repairedName;
          canonicalAsset.semanticScore = mustRepairSemantics ? .98 : semanticScore;
          canonicalAsset.qualityScore = Math.max(Number(canonicalAsset.qualityScore ?? 0), .92);
        }
        if (mustRepairSemantics) {
          attributions[index] = {
            ...attributions[index],
            id: `premium-hmo:${index + 1}`,
            source: "curated-hmo-gallery",
            sourceUrl: `/premium-hmo/scene-${String(index + 1).padStart(2, "0")}.mp4`,
            fallback: false,
            query: "curated HMO property footage",
            enhancement: "semantic-repair"
          };
        }
      }
    }
    ...
    if (qualityScorecard.decision !== "accept" && qualityScorecard.overall < (job.input.workflow ===  long-form ? 85 : 90)) {
