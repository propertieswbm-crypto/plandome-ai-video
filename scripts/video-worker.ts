...
import { writeCanvaStoryboard, writeComposition, type PlannedScene } from "./video-composition";
...
async function main() {
  ...
  try {
    ...
    await update(job, "composing", job.input.useAvatar ? 57 : 42, "Finding UK visuals and building ad typography"); await copyFile(path.join(root, "apps/web/public/brand/plandome-logo.png"), path.join(assets, "logo.png")); await hydratePreviousSceneVisuals(id,job.input.script,assets,seed,scenes.length);
    ...
    const videoScene = lines.findIndex((line, index) => /victorian|heritage|period|historic london/i.test(line) && scenes[index].kind !== "avatar" && !scenes[index].videoAsset && !scenes[index].visualAsset); if (videoScene >= 0) { try { const asset = "uk-victorian-broll.webm"; const target = path.join(assets, asset); const cacheDir = path.join(root, ".data", "media-cache"); const cached = path.join(cacheDir, asset); await mkdir(cacheDir, { recursive: true }); try { await copyFile(cached, target); attributions.push({ title: "A Trip Through The Streets of London, Sep 26, 1917", source: "https://commons.wikimedia.org/wiki/File:A_Trip_Through_The_Streets_of_London,_Sep_26,_1917.webm", license: "Public domain" }); } catch { attributions.push(await fetchUkVictorianVideo(seed, cached)); await copyFile(cached, target); } scenes[videoScene].videoAsset = asset; } catch { /* line-matched image fallback below */ } }
    const usedSources = new Set<string>(attributions.map((item) => String((item as { source?: string }).source ?? "")).filter(Boolean));
    for (let index = 0; index < scenes.length; index++) { if (scenes[index].kind === "avatar" || scenes[index].kind === "cta" || scenes[index].kind === "pack" || scenes[index].videoAsset || scenes[index].visualAsset) continue; const asset = `uk-visual-${index}.jpg`; const queries = visualQueryTiers(lines[index], scenes[index].brief); let matched = false; for (let attempt = 0; attempt < queries.length && !matched; attempt++) { try { attributions.push(await fetchUkVisual(queries[attempt], seed + index * 17 + attempt, path.join(assets, asset), usedSources)); scenes[index].visualAsset = asset; scenes[index].visualFailure = undefined; matched = true; } catch (cause) { scenes[index].visualFailure = cause instanceof Error ? cause.message : "Unknown visual provider error."; } } }
